    ...
    try:
        ...
        if not has_content:
            ...
            if settings.FEATURES.get('ENTRANCE_EXAMS', False) and getattr(course, 'entrance_exam_enabled', False):
                exam_chapter, _ = get_entrance_exam_content_info(request, course_module, course)
                if exam_chapter is not None:
                    exam_section = None
                    if len(exam_chapter.get_display_items()):
                        exam_section = exam_chapter.get_display_items()[0].url_name
        ...
        if settings.FEATURES.get('ENTRANCE_EXAMS', False) and getattr(course, 'entrance_exam_enabled', False):
            _, exam_current_score = get_entrance_exam_content_info(request, course_module, course)
            if exam_current_score is not None:
                context['entrance_exam_current_score'] = exam_current_score
